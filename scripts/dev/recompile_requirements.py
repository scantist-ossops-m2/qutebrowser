...
def convert_line(line, comments):
    ...
    pkgname = normalize_pgk(line.split('=')[0])
    ...
    for line in fobj:
        if line.startswith('#@'):
            ...
            if command == 'filter':
                ...
                comments['filter'][normalize_pgk(pkg)] = filt
            elif command == 'comment':
                pkg, comment = args.split(' ', maxsplit=1)
                comments['comment'][normalize_pgk(pkg)] = comment
            elif command == 'ignore':
                comments['ignore'] += [normalize_pgk(a) for a in args.split(', ')]
                ...
                comments['markers'][normalize_pgk(pkg)] = markers
            elif command == 'add':
...
def parse_versioned_line(line):
    ...
    return normalize_pgk(name), version
